...
const EscalatedChainsList = ({ chains }: { chains: EscalatedChain[] }) => {
	...
	const chainsByEmployee = currentData.reduce(
		(acc, chain) => {
			...
			<div className="max-w-5xl mx-auto">
				...
				<div className="grid gap-5">
					{Object.keys(chainsByEmployee).length > 0 ? (
						Object.entries(chainsByEmployee).map(
							([employeeId, employeeChains]) => (
								<div
									key={employeeId}
										...
										{employeeChains[0] && (
											<div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-100 dark:border-gray-700">
												<div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-700 dark:text-gray-300 text-sm">
													...
													<p className="flex flex-col">
														...
														<span className="font-medium text-red-600 dark:text-red-400">
															{employeeChains[0].escalation_reason}
														</span>
													</p>
													<p className="flex flex-col md:col-span-2">
														...
														<span className="font-medium text-gray-900 dark:text-white">
															{formatDate(employeeChains[0].escalated_at)}
														</span>
													</p>
												</div>
												...
												<div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700">
													<div className="flex items-center justify-between mb-3">
														<div className="flex items-center">
															...
															<span className="text-xs bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300 py-0.5 px-2 rounded-full font-medium">
																{employeeChains.reduce(
																	(total, chain) =>
																		total + chain.session_ids.length,
																	0
																)}
															</span>
														</div>
													...
													<div
														className={`transition-all duration-300 ${expandedEmployees[employeeId] ? 'opacity-100 max-h-72 overflow-y-auto' : 'opacity-0 max-h-0 overflow-hidden'}`}
													>
														{expandedEmployees[employeeId] && (
