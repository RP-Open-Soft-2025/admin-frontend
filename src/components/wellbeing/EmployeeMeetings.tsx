  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h3 className="text-xl font-semibold text-gray-800 dark:text-white/90">Employee Meetings</h3>
        ...
        {meetingsData.map((meeting) => (
          ...
          >
            <div className="flex items-center justify-between">
              <h4 className="text-base font-semibold text-gray-900 dark:text-white">{meeting.title}</h4>
              <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${getPriorityBadge(meeting.priority)}`}>
                {meeting.priority}
              </span>
            </div>
            
            <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">{meeting.purpose}</p>
