...
const DepartmentHeatmap: React.FC = () => {
  ...
  const sortedDepartments = [...departmentAverages].sort((a, b) => b.averageScore - a.averageScore)
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
      <div className="mb-6">
        <h3 className="text-xl font-semibold text-gray-800 dark:text-white/90">Department Wellbeing</h3>
            ...
            <div className="mt-1 flex items-center gap-2">
              <div 
                className={`h-2.5 w-2.5 rounded-full ${
                  dept.averageScore >= 75 ? 'bg-green-500' : 
                  dept.averageScore >= 50 ? 'bg-amber-500' : 
                  dept.averageScore >= 25 ? 'bg-orange-500' : 'bg-red-500'
                }`}
              ></div>
              <span className="text-base font-semibold text-gray-900 dark:text-white">
                {dept.averageScore}
              </span>
              <span className="text-xs text-gray-500">({dept.employeeCount} emp.)</span>
            </div>
          </div>
        ))}
      </div>
