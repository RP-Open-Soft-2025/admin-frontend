...
const PriorityEmployees: React.FC = () => {
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
      <div className="mb-6">
        <h3 className="text-xl font-semibold text-gray-800 dark:text-white/90">Priority Employees</h3>
        ...
        <table className="w-full min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
            {priorityEmployees.map(employee => (
              <tr key={employee.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.02]">
                ...
                <td className="whitespace-nowrap px-3 py-4 text-sm">
                  <div className={`font-medium ${employee.vibemeterScore < 25 ? 'text-red-600' : 'text-orange-500'}`}>
                    {employee.vibemeterScore}/100
                  </div>
                </td>
                <td className="whitespace-nowrap px-3 py-4 text-sm">
                  <span className={`inline-flex rounded-full px-2 text-xs font-semibold leading-5 ${getRiskBadgeColor(employee.riskLevel)}`}>
                    {employee.riskLevel}
                  </span>
                </td>
