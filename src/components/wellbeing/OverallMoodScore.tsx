...
const OverallMoodScore: React.FC = () => {
	...
	const trendPercentage = Math.round(
		((metrics.avgMoodScore - prevScore) / prevScore) * 100
	)
		...
		<div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
			<div className="mb-4 flex items-center justify-between">
				...
				<div className="flex items-center gap-2">
					<span
						className={`text-lg font-medium ${isPositiveTrend ? 'text-green-500' : 'text-red-500'}`}
					>
						{isPositiveTrend ? '+' : ''}
						{trendPercentage}%
					</span>
			...
			<div className="mt-4 flex justify-between">
				...
				<div className="text-center">
					...
					<p className="mt-1 text-xl font-medium text-orange-500">
						{metrics.highRiskCount}
					</p>
				</div>
				<div className="text-center">
					<p className="text-sm text-gray-500 dark:text-gray-400">At Risk</p>
					<p className="mt-1 text-xl font-medium text-yellow-500">
						{metrics.atRiskPercentage}%
					</p>
				</div>
				<div className="text-center">
					<p className="text-sm text-gray-500 dark:text-gray-400">Total</p>
					<p className="mt-1 text-xl font-medium text-blue-500">
