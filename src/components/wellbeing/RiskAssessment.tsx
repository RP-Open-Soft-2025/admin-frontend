...
const RiskAssessment: React.FC = () => {
  ...
  const criticalCount = employeeData.filter(emp => emp.riskLevel === 'Critical').length
  const highCount = employeeData.filter(emp => emp.riskLevel === 'High').length
  const mediumCount = employeeData.filter(emp => emp.riskLevel === 'Medium').length
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
      <div className="mb-4">
        <h3 className="text-xl font-semibold text-gray-800 dark:text-white/90">Risk Assessment</h3>
