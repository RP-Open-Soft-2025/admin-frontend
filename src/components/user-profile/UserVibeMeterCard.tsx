			...
			<div className="flex flex-col">
				...
				<div className="flex flex-col items-center justify-center p-6">
					<div className="mb-6 text-center">
						<div className={`rounded-full ${getColorForScore(latestVibe.Vibe_Score)} w-32 h-32 flex items-center justify-center shadow-lg`}>
							<span className="text-white text-4xl font-bold">{normalizedScore.toFixed(1)}</span>
						</div>
						
						<h3 className={`text-2xl font-bold mt-4 ${getTextColorForScore(latestVibe.Vibe_Score)}`}>
					...
					<div className="mt-6 flex flex-col items-center w-full max-w-md">
						...
						<div className="flex justify-between w-full px-0 relative">
							{[1, 2, 3, 4, 5].map((value) => {
								...
								const isActive = value === 5 && isScoreFive ? true : 
												 value <= Math.ceil(normalizedScore);
								return (
									<div key={value} className="flex flex-col items-center">
										<div 
											className={`w-4 h-4 rounded-full mb-1 ${
												isActive ? getColorForPoint(value) : 'bg-gray-200 dark:bg-gray-700'
											}`}
										></div>
										<div className={`text-xs font-medium ${
											isActive ? getTextColorForPoint(value) : 'text-gray-500 dark:text-gray-400'
										}`}>
											{value}
										</div>
									</div>
					...
					<div className="mt-8 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg w-full max-w-md">
						<div className="flex justify-between mb-2">
							...
							<p className={`text-sm font-semibold ${getTextColorForScore(latestVibe.Vibe_Score)}`}>
