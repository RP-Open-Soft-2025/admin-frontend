// Types
export type Department = 'Engineering' | 'Marketing' | 'Sales' | 'HR' | 'Finance' | 'Operations' | 'Product' | 'Design';
export type Role = 'Manager' | 'Director' | 'Team Lead' | 'Junior' | 'Senior' | 'Intern' | 'VP' | 'C-Level';
...
export const calculateOverallMetrics = () => {
  ...
  const avgMoodScore = employeeData.reduce((sum, emp) => sum + emp.vibemeterScore, 0) / totalEmployees;
  const avgEngagementRate = employeeData.reduce((sum, emp) => sum + emp.engagementRate, 0) / totalEmployees;
  
  const criticalRiskCount = employeeData.filter(emp => emp.riskLevel === 'Critical').length;
  const highRiskCount = employeeData.filter(emp => emp.riskLevel === 'High').length;
  ...
  return {
    ...
    criticalRiskCount,
    highRiskCount,
    atRiskPercentage: Math.round(((criticalRiskCount + highRiskCount) / totalEmployees) * 100)
  };
};
